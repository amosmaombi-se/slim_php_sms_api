...
--
-- Host: localhost
-- Generation Time: Nov 26, 2019 at 12:33 AM
...
('356a192b7913b04c54574d18c28d46e6395428ab', 'ebfcc5fc99b201fc1593deddbe74f325669e2efd0422cfa4e6c9beeb57f41268', 1574775015);
...
CREATE TABLE `tb_course` (
  ...
  `c_shortname` varchar(30) NOT NULL,
...
INSERT INTO `tb_course` (`c_code`, `c_name`, `c_shortname`, `c_description`) VALUES
...
('ITT04101', 'FUNDAMENTALS OF COMPUTER', 'FUNDAMENTALS OF COMPUTER', 'BLA BLA BLA'),
...
INSERT INTO `tb_program` (`prog_id`, `name`, `s_name`, `length`, `l_unit`, `description`) VALUES
('PROG005', 'Information & Communication Technology', '', 0, '', ''),
('PROG006', 'Childcare & Education ', '', 0, '', ''),
('PROG007', 'Front office operations in hotel management', '', 0, '', ''),
('PROG008', 'Busness Operation Assistant in Busness Adminstration', 'BOA BA', 1, 'year', 'Hakuna mamtata');
...
('BEAC/EL/01/001', 'ODILO', 'ANDREA', 'AMMA', '30.07.1997', 'Tanzanian', 'Male', '0655007457', '15.08.2018', '', 'PROG007'),
('BEAC/FO/01/013', 'SOPHIA', 'JOHN', 'JOSEPH', '23.02.2001', 'Tanzanian', 'female', '0718617461', '15.08.2018', '', 'PROG007');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tb_course_view`  AS  select `tb_course`.`c_code` AS `c_code`,`tb_course`.`c_name` AS `c_name`,`tb_course`.`c_shortname` AS `c_shortname`,`tb_program`.`prog_id` AS `prog_id`,`tb_program`.`name` AS `name` from ((`tb_program` join `tb_program_course`) join `tb_course`) where ((`tb_program`.`prog_id` = `tb_program_course`.`program_id`) and (`tb_course`.`c_code` = `tb_program_course`.`course_id`)) ;
